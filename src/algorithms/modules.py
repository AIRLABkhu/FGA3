...
class Encoder(nn.Module):
	...
	def shared(self,x,detach=False):

		return self.shared_cnn.latent(x)
...
class Actor(nn.Module):
	...
	def show_headmap(self,x,detach=False):
		result=self.encoder.shared(x,detach)
		result=result[0,:,:,:]
		result=torch.mean(result,axis=0)
		return result

	def show_latent(self,x,detach=False):
		result=self.encoder(x,detach)
		result=result[0,:,:,:]
		result=torch.mean(result,axis=0)
		return result
